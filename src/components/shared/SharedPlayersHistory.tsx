...
import { Player, GamePlayer } from '@/types/poker';

interface GameHistoryItem extends Omit<GamePlayer, 'player'> {
  games: {
    date: string;
...
const SharedPlayersHistory: React.FC<SharedPlayersHistoryProps> = ({ token, playerId }) => {
  ...
  const filteredHistory = useMemo(() => {
    const res = gameHistory.filter(g => selectedMonth === 'all' || format(new Date(g.games.date), 'MMM yyyy') === selectedMonth);
    return res.sort((a, b) => {
      let valA = 0, valB = 0;
      switch (sortField) {
