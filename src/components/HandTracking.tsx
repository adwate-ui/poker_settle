...
import { Play, CheckCircle, TrendingUp, Trophy, X, ChevronDown, ChevronUp, Sparkles } from 'lucide-react';
...
const HandTracking = ({ game, positionsJustChanged = false, onHandComplete, initialSeatPositions = [] }: HandTrackingProps) => {
  ...
  // Visual pot size calculation: total pot minus chips currently sitting on the table in front of players
  ...
  const finishHand = async (winnerIds: string[], finalStageOverride?: HandStage, lastAction?: PlayerAction) => {
    ...
    // Prepare actions with hole cards
    const finalActions = actionsToSave.map(action => {
      // If this is the last action for this player, attach hole cards
    ...
    // Save everything in one go (or 3 requests total: Hand, Actions, Complete)
    await saveCompleteHandData(
      savedHand.id,
      finalActions,
      streetCardsData,
      winnerIds,
    ...
    <>
      ...
      {isMobile && (
        <Dialog open={showCardSelector} onOpenChange={(isOpen) => {
          ...
          <DialogContent className="w-[95vw] max-h-[90vh] overflow-y-auto glass-panel border-white/10 shadow-2xl p-4">
            <DialogHeader>
              <DialogTitle className="show-xl font-heading font-bold text-luxury-primary flex items-center gap-2">
                ...
                return (
                  <div key={suit.code} className="space-y-2">
                    <div className="flex items-center gap-2 pb-1 border-b border-white/10">
                      ...
                      <h3 className="font-luxury font-bold text-base tracking-wide">{suit.name}</h3>
                    </div>
                      ...
                      {ranks.map(rank => {
                        ...
                        return (
                          <button
                            ...
                            className={cn(
                              "relative aspect-[5/7] w-full transition-all duration-200 rounded touch-manipulation",
