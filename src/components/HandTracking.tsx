...
const HandTracking = ({ game, positionsJustChanged = false, onHandComplete, initialSeatPositions = [] }: HandTrackingProps) => {
  ...
  const handleEditCards = () => {
    ...
    if (cards) {
      setCardSelectorType(engine.stage as any);
  ...
  const handleAction = async (type: any, amount?: number) => {
    await engine.recordAction(type, amount);
    setBetAmount('');
    setCardsJustAdded(false);
  };

  const handleFinishHand = async (winnerIds: string[], stageOverride?: any) => {
  ...
  if (engine.stage === 'showdown') {
    ...
    return (
      <div className="mt-6">
        {isMobile ? (
          <Drawer open={true}><DrawerContent className="h-[90vh] p-4 overflow-y-auto">{content}</DrawerContent></Drawer>
        ) : content}
        <CardSelector
